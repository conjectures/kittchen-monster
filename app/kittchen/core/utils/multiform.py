...
class MultiFormMixin(ContextMixin):
    ...
    def get_forms(self, form_classes, form_names=None, bind_all=False):
        return dict([(key, self._create_form(
            key, klass, (form_names and key in form_names) or bind_all)) for key, klass in form_classes.items()])
    ...
    def get_form_kwargs(self, form_name, bind_form=False):
        ...
        # kwargs.update({'queryset': self.get_queryset(form_name)})
    ...
    @debug
    def get_queryset(self, form_name):
        return self.queryset.get(form_name, None)
